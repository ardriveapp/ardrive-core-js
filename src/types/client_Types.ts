...
export interface ILocalDriveEntity<P extends drivePrivacy> {
	id?: number | invalid; // an identifier that can be used in any underlying database
	owner?: string; // the public arweave wallet address that owns this drive
	entity?: PrivacyToDriveEntity<P> | invalid; // The underlying ArFS Drive entity and metadata
	isLocal?: number | invalid; // Indicates if the drive is being synchronized locally or not.  0 for "no", 1 for "yes"
}

export class ArFSLocalDriveEntity<P extends drivePrivacy> implements ILocalDriveEntity<P> {
	id: number | invalid = invalid;
	owner: string = invalid;
	entity: PrivacyToDriveEntity<P> | invalid = invalid;
	isLocal: number | invalid = invalid;
...
export class ArFSLocalPublicDriveEntity extends ArFSLocalDriveEntity<drivePrivacy.PUBLIC> {
	entity: ArFSPublicDriveEntity | invalid = invalid;
}

export class ArFSLocalPrivateDriveEntity extends ArFSLocalDriveEntity<drivePrivacy.PRIVATE> {
	entity: ArFSPrivateDriveEntity | invalid = invalid; // The underlying ArFS Drive entity and metadata
}
...
export interface ILocalMetaData<P extends drivePrivacy> {
	...
	isLocal?: number | invalid;
	entity?: PrivacyToFileFolderEntity<P> | invalid;
	data?: PrivacyToData<P> | invalid;
}

export class ArFSLocalMetaData<P extends drivePrivacy> implements ILocalMetaData<P> {
	id: number | invalid = invalid; // an identifier that can be used in any underlying database, eg. 1, 2, 3 etc.
	...
	size: number | invalid = invalid; // The size in bytes of the underlying file data
	version: number | invalid = invalid; // The version number of the underlying file data.  Should be incremented by 1 for each version found for a given fileId.
	isLocal: number | invalid = invalid; // Indicates if the drive is being synchronized locally or not.  0 for "no", 1 for "yes"
	entity: PrivacyToFileFolderEntity<P> | invalid = invalid;
	data?: PrivacyToData<P> | invalid = invalid;
...
export class ArFSLocalPublicFolder extends ArFSLocalMetaData<drivePrivacy.PUBLIC> {
	entity: ArFSPublicFileFolderEntity | invalid = invalid; // The underlying ArFS Entity
}

export class ArFSLocalPrivateFolder extends ArFSLocalMetaData<drivePrivacy.PRIVATE> {
	entity: ArFSPrivateFileFolderEntity | invalid = invalid; // The underlying ArFS Entity
}
...
export class ArFSLocalPublicFile extends ArFSLocalMetaData<drivePrivacy.PUBLIC> {
	entity: ArFSPublicFileFolderEntity | invalid = invalid;
	data: ArFSPublicFileData | invalid = invalid;
}

export class ArFSLocalPrivateFile extends ArFSLocalMetaData<drivePrivacy.PRIVATE> {
	entity: ArFSPrivateFileFolderEntity | invalid = invalid;
	data: ArFSPrivateFileData | invalid = invalid;
}
