...
export class ArFSLocalMetaData {
	...
	static From({ id, owner, hash, path, size, version, isLocal }: ArFSLocalMetaDataArguments) {
		return new ArFSLocalMetaData(
			id ?? 0,
			owner ?? guards.emptyString,
			hash ?? guards.emptyString,
			path ?? guards.emptyString,
			size ?? 0,
			version ?? 0,
			isLocal ?? 1
		);
...
export class ArFSLocalFile extends ArFSLocalMetaData {
	...
	static From({
		...
		return new ArFSLocalFile(
			...
			entityType ?? guards.emptyString,
			name ?? guards.emptyString,
			syncStatus ?? 0,
			txId ?? guards.emptyString,
			unixTime ?? 0,
			dataContentType ?? guards.emptyString,
			dataSyncStatus ?? 0,
...
export class ArFSLocalPrivateFile extends ArFSLocalMetaData {
	...
	static From({
		...
		return new ArFSLocalPrivateFile(
			...
			dataTxId ?? guards.emptyString,
			dataUnixTime ?? 0,
			cipher ?? guards.emptyString,
			cipherIV ?? guards.emptyString,
			dataCipher ?? guards.emptyString,
