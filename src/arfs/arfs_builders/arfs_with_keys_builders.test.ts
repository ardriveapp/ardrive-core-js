...
import { FolderHierarchy } from '../../exports';
...
import { ArFSPrivateFileOrFolderWithPathsAndKeysBuider } from './arfs_with_keys_builders';
...
describe('ArFSPrivateFileOrFolderWithPathsAndKeysBuider class', () => {
	...
	it('Built folders will have the extra driveKey attribute set', async () => {
		const builder = new ArFSPrivateFileOrFolderWithPathsAndKeysBuider(stubFolder_0, stubHierarchy, driveKey);
		const folderWithKeys = await builder.build();
	...
	it('Built files will have the driveKey and fileKey attributes set', async () => {
		const builder = new ArFSPrivateFileOrFolderWithPathsAndKeysBuider(stubFile, stubHierarchy, driveKey);
		const fileWithKeys = await builder.build();
