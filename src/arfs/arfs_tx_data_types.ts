...
export abstract class ArFSFolderMetadataTransactionData implements ArFSObjectTransactionData {
	abstract asTransactionData(): string | Buffer;
	sizeOf(): ByteCount {
		return new ByteCount(this.asTransactionData().length);
	}
...
export class ArFSPublicFolderMetadataTransactionData extends ArFSFolderMetadataTransactionData {
	constructor(
		private readonly name: string,
		private readonly size: ByteCount,
		private readonly lastModifiedDate: UnixTime,
...
export class ArFSPrivateFolderMetadataTransactionData extends ArFSFolderMetadataTransactionData {
	private constructor(
		readonly cipher: CipherType,
		readonly cipherIV: CipherIV,
		readonly encryptedFolderMetadata: Buffer,
		readonly fileKey: FileKey,
		readonly driveAuthMode: DriveAuthMode = 'password'
	) {
		super();
	...
	static async from(
		...
		fileId: FileID,
		driveKey: DriveKey
	): Promise<ArFSPrivateFolderMetadataTransactionData> {
		const fileKey: FileKey = await deriveFileKey(`${fileId}`, driveKey);
		const { cipher, cipherIV, data }: ArFSEncryptedData = await fileEncrypt(
			fileKey,
			Buffer.from(
		...
		return new ArFSPrivateFolderMetadataTransactionData(cipher, cipherIV, data, fileKey);
	}

	asTransactionData(): Buffer {
		return this.encryptedFolderMetadata;
	}
}
