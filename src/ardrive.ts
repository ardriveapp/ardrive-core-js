...
export class ArDrive extends ArDriveAnonymous {
	...
	public async createPublicDrive({ driveName }: CreatePublicDriveParams): Promise<ArFSResult> {
		...
		const rootFolderMetaDataPrototype = new ArFSPublicFolderMetaDataPrototype(
			new ArFSPublicFolderTransactionData(driveName),
			fakeEntityId,
			fakeEntityId
		);
		const driveMetaDataPrototype = new ArFSPublicDriveMetaDataPrototype(
			new ArFSPublicDriveTransactionData(driveName, fakeEntityId),
			fakeEntityId
		);
		const { rewardSettings, totalWinstonPrice } = await this.costEstimator.estimateCreateDrive({
	...
	public async createPrivateDrive({ driveName, newPrivateDriveData }: CreatePrivateDriveParams): Promise<ArFSResult> {
		// Use stub data to estimate costs since actual data requires entity IDs generated by ArFSDao
		const rootFolderMetaDataPrototype = new ArFSPrivateFolderMetaDataPrototype(
			fakeEntityId,
			fakeEntityId,
			await ArFSPrivateFolderTransactionData.from(driveName, newPrivateDriveData.driveKey)
		);
		const driveMetaDataPrototype = new ArFSPrivateDriveMetaDataPrototype(
			fakeEntityId,
