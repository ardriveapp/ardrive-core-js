...
import { cipherType, contentType, entityType, syncStatus, yesNoInteger } from '../types/type_guards';
...
export async function getFileMetaDataFromTx(fileDataTx: GQLEdgeInterface, user: ArDriveUser) {
	const fileToSync: ArFSFileMetaData = new ArFSFileMetaData({
		...
		parentFolderId: '',
		fileVersion: 0,
		fileDataSyncStatus: syncStatus.READY_TO_DOWNLOAD,
		fileMetaDataSyncStatus: syncStatus.READY_TO_DOWNLOAD,
		login: user.login,
		isLocal: yesNoInteger.NO,
		isPublic: yesNoInteger.NO,
		cloudOnly: yesNoInteger.NO
	});
	try {
