...
function createData(data, signer, opts) {
    ...
    (0, assert_1.default)(_owner.byteLength == signer.ownerLength, new Error(`Owner must be ${signer.ownerLength} bytes, but was incorrectly ${_owner.byteLength}`));
    ...
    if (_target) {
        (0, assert_1.default)(_target.byteLength == 32, new Error("Target must be 32 bytes but was incorrectly ${_target.byteLength}"));
    ...
    if (_anchor) {
        ...
        (0, assert_1.default)(_anchor.byteLength == 32, new Error("Anchor must be 32 bytes"));
